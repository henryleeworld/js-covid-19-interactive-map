...
am4core.ready(function() {
    ...
    sizeLabel.tooltipText = "一些國家的案例太多，即使它們之間存在顯著差異，但對於較小值的國家來說，氣泡往往看起來相同。此滑桿可用於增加氣泡的最大大小，以便當您放大具有相對較小值的區域時，無論如何都可以比較案例。"
    sizeLabel.fill = am4core.color("#ffffff");
    ...
    filterLabel.tooltipText = "此過濾器允許從地圖中刪除案例較多的國家，以便可以比較案例較少的國家。"
    filterLabel.fill = am4core.color("#ffffff");
    ...
    seriesTypeSwitch.leftLabel.text = "全部";
    seriesTypeSwitch.rightLabel.text = "日更"
    ...
    function addSeries(name, color) {
        ...
        series.tooltipText = "全部 {name}: {valueY}";
    ...
    label.text = "在各國提交數據之前，當日統計數據可能不完整。";
    ...
    function changeDataType(name) {
        currentType = name;
        currentTypeName = translation(name);
        if (name != "deaths") {
            currentTypeName += "病例數";
        }
    ...
    function updateTotals(index) {
        if (!isNaN(index)) {
            ...
            for (var key in buttons) {
                ...
                if (!isNaN(count)) {
                    buttons[key].label.text = translation(key) + ": " + numberFormatter.format(count, '#,###');
                }
            }
            currentIndex = index;
        }
